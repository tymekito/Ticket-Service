<template>
  <div>
    ...
    <v-card max-width="1200" min-height="860" class="mx-auto booksContainer">
      <div class="basePageHeaderText">My Books</div>
          ...
          <v-col v-for="book in filteredIteams" :key="book.id" :cols="6">
            <v-card @click="onBookClick(book)">
              ...
              <v-card-actions>
                ...
                <b class="ml-1 mr-1"  >Date of delivery</b>
                <span>{{ book.expirationDate }}</span>
                <v-spacer></v-spacer>
              </v-card-actions>
            </v-card>
          </v-col>
      ...
      <BookDetails ref="BookDetails" @deleted="onReturnBook" />
    </v-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions("bookList", ["getBookList", "returnBook"]),
    ...mapActions("login", ["refreshUserData"]),

    async onReturnBook() {
      await this.returnBook(this.selectedBook.id);
    ...
    async refreshPageData() {
      await this.getBookList(this.userDetails.userId);
      this.books = this.bookList;
      this.alert = false;
    },
  },
  computed: {
    ...mapGetters("bookList", ["bookList"]),
